...
class Config:
    ...
    def _initialize(self):
        ...
        self.IGNORE_VERSION_HISTORY = (
            os.getenv("IGNORE_VERSION_HISTORY", "False").lower() == "true"
        )
        self.ALLOW_SCHEMA_MISMATCH = (
            os.getenv("ALLOW_SCHEMA_MISMATCH", "True").lower() == "true"
        )
        self.ALLOW_SMALLER_VERSION_LIMIT_ON_DESTINATION = (
            os.getenv("ALLOW_SMALLER_VERSION_LIMIT_ON_DESTINATION", "True").lower()
            == "true"
        )
        self.INCLUDE_ITEM_PERMISSIONS = (
            os.getenv("INCLUDE_ITEM_PERMISSIONS", "False").lower() == "true"
        )
        self.BYPASS_SHARED_LOCK = (
            os.getenv("BYPASS_SHARED_LOCK", "True").lower() == "true"
        )
        self.MOVE_BUT_KEEP_SOURCE = (
            os.getenv("MOVE_BUT_KEEP_SOURCE", "False").lower() == "true"
        )
